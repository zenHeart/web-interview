...
:::caution 核心红线：禁止在 `<Answer>` 内部使用 Markdown 标题（# ~ ######）
页面的右侧目录（Table of Contents）仅用于导航具体的面试题（即所有 `## 题目`）。
**在 `<Answer>` 标签内部，绝对禁止使用任何级别的 Markdown 标题（`#`、`##`、`###`、`####` 等）！**
如果使用了 `### 核心结论` 或 `### 原理解析`，Docusaurus 会将其解析为子级目录挂在题目下方，导致右侧 TOC 目录严重污染！
**正确做法**：答案内部的所有区块分隔必须使用加粗文本且以冒号结尾，例如：
- `**核心结论:**` 或 `**核心概念:**`
- `**原理解析:**` 或 `**机制说明:**`
- `**示例说明:**` 或 `**规范实现:**`
- `**面试官视角:**`
...
标准金牌范例请严格对照：[`docs/js/type-value`](/docs/js/type-value)。
:::

:::tip 题目标题拟定黄金法则（什么才叫面试题）
题库的本质是**面试模拟与能力定级**，绝不是技术手册的章节摘要。严禁生成“知识点拼盘”或“学术论文大纲式”伪题目！
1. **真实面试口吻（Authentic Voice）**：题目标题必须是面试官在真实面试中能自然说出口的问题，避免堆砌英文术语和括号注释。
   - ❌ 错误：`## 大型超万行前端遗留系统（Legacy）架构腐化治理与渐进式重构（Strangler Fig 绞杀者模式）实战？`
   - ✅ 正确：`## 面对超万行严重腐化的前端遗留系统，如何用绞杀者模式进行渐进式重构？`
2. **单一职责原则（SRP）**：每道题只聚焦一个核心考点，严禁用顿号/与/和拼接多个不相关的 API 或概念。
   - ❌ 错误：`## Promise.try()、Promise.withResolvers()、Iterator Helpers 与 Set 集合代数？`
   - ✅ 正确：拆分为 `## Promise.try() 解决了传统 Promise.resolve().then() 的什么痛点？` 等独立真题。
3. **疑问与情境引导**：必须有明确的疑问代词（为什么、如何、区别）或具体的代码反直觉场景（如 `0.1 + 0.2 !== 0.3`），拒绝陈述句后简单挂问号。
:::
...
# ------------------------------
# 题目头部区域（唯一标识 + ID，全站统一使用 ## 与小写 slug）
# ------------------------------
...
# ------------------------------
# 自定义组件 - 代码执行与交互可视化组件：TestCode / Sandpack / CustomSandPack / Visualizer 等
# ------------------------------
<ComponentUsage> ::=  <SandpackComponent> | <TestCodeComponent> | <CustomSandPackComponent> | <VisualizerComponent>

<SandpackComponent> ::= "<Sandpack template=" <StringLiteral>
                         " options=" <OptionsObject>? " files=" <FilesObject> " />"

<CustomSandPackComponent> ::= "<CustomSandPack template=" <StringLiteral>?
                               " options=" <OptionsObject>? " files=" <FilesObject> " />"

<TestCodeComponent> ::= "<TestCode" (" options=" <OptionsObject>)?
                         " files=" <FilesObject> " />"

<VisualizerComponent> ::= "<ViteHmrVisualizer />" 
                        | "<StranglerVisualizer />" 
...
先阅读[好问题的标准](/interview-guide/bloom-taxonomy-questions) 理解问题是否值得被收录。
...
参考 [docusaurus 交互式代码](https://docusaurus.io/docs/markdown-features/code-blocks#interactive-code-editor)
...
参考 [sandpack](https://sandpack.codesandbox.io/docs/getting-started/usage) 使用
...
用在使用 html 说明功能的场景，或者使用 ESM 说明指定版本
Vue、React 等框架或工具场景

import StaticDemo from '!!raw-loader!./answers/static.html'
...
import NodeDemo from '!!raw-loader!./answers/node.mjs'
...
<TabItem value="testcode">

用在测试验证的场景，通常在代码编写提或者

import testcodeDemoAdd from '!!raw-loader!./answers/testcode/add.js'
import testcodeDemoAddTest from '!!raw-loader!./answers/testcode/add.test.js'

<TestCode
   files={{
      "/add.test.js": testcodeDemoAddTest,
      "/add.js": testcodeDemoAdd,
   }}
/>
...
- 单文件：使用问题 ID 作为文件名，扩展名按类型选择（js/ts/mjs/
   html/css/json 等）。例如问题头为 `## ... {#P0-tab-communicate}`，
   则示例为 `answers/tab-communicate.js`。
- 多文件：以问题 ID 作为文件夹名，内部放置最小必需文件集。
   例如 `answers/tab-communicate/index.js`、
   `answers/tab-communicate/worker.js`。
- MVP 原则：仅保留支撑核心概念的最小代码；在核心逻辑处添加简要注释；
   可运行优先（便于 TestCode/Sandpack）；避免与核心无关的样板代码。
- files 用法：在组件 `files` 中引用时，键需以 `/` 开头并与导入内容对应。
   例如 `{ "/index.js": demo }`。

### 面试官视角

每道题附带 评分 Rubric（要点清单 + 加分项）。保证信度，减少面试官主观性。
候选人也能知道什么是**标准答案**，自我检测更精准。
