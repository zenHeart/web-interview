...
export default function extractQuestionsPlugin (
  ...
  return {
    ...
    async loadContent () {
      ...
      const exclude: string[] = siteConfig.presets.find((preset) => (preset as unknown[]).includes?.('classic'))?.[1]?.docs?.exclude || []
      ...
      // 使用 fast-glob 查找所有 markdown 文件
        ...
        const questions = await Promise.all(
          files.map(async (filePath) => {
            ...
            const title = matches ? matches[0].slice(3).trim() : path.basename(filePath, path.extname(filePath))
