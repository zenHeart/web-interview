...
describe ('this 绑定', function () {
  ...
  describe ('eval 绑定', function () {
    ...
    it ('eval 采用外层执行环境的 this', function () {
      ...
      let res = eval ('this');
      let res1 = eval ('(function a(){return this})()');
  ...
  describe ('with 绑定', function () {
    // 实际上可以等效将 eval 的代码放入当前执行环境则即可知道实际运行结果
    it ('with 只演示', function () {
	  let a,obj={
		  say() {
			  return this;
		  }
...
describe ('绑定优先级', function () {
  it ('new 优先级高于显示绑定', function () {
    ...
    let obj = {a: 1};
    let Person1 = Person.bind (obj);
    let tom = new Person1 ('tom');
