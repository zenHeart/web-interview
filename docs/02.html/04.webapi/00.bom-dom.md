...
<Answer>
BOM 是 Browser Object Model 的缩写，中文一般叫“浏览器对象模型”。

它描述的是浏览器提供给 JavaScript 的一组对象，这些对象让 JS 能够控制浏览器本身的一些功能，比如 URL 导航、历史记录、浏览器窗口大小、屏幕信息、弹窗、定时器等。和 DOM（Document Object Model）不完全一样，BOM 更多是与浏览器环境、浏览器窗口（tab）、浏览器与页面之外的交互。

BOM 没有一个像 DOM 那样被 W3C 或 WHATWG 明确统一标准化的规范。也就是说，不同浏览器对 BOM 中某些功能、属性或方法的支持可能不完全相同。

常见的核心能力包括

|对象|主要作用|
|---|---|
|**window**|顶层/全局对象: `alert()/ confirm()/ prompt()`; 窗口尺寸 `innerWidth/innerHeight/ outerWidth/outerHeight`; `open()/ close()`|
|**navigator**|环境信息: `userAgent、language、platform、onLine、cookieEnabled`|
|**location**|URL 读写: `href protocol host pathname search hash assign replace reload`|
|**history**|历史记录控制: back forward go pushState replaceState length|
|**screen**|屏幕信息: width height availWidth availHeight pixelDepth|
|**frames**|访问当前窗口内全部 frame/iframe 集合|
|**setTimeout/setInterval**|定时/循环任务调度 clearTimeout cle arInterval|
...
## 说下 IntersectionObserver API ？{#P1-intersection-observer}

<Answer>

**核心概念**
`IntersectionObserver`（交叉观察器）是现代浏览器提供的高性能异步观察 API，用于监听一个目标元素是否出现在某个根元素（或顶级视口）的可视区域内：
1. **解决痛点**：传统监听滚动事件（`scroll`）结合 `getBoundingClientRect()` 会频繁触发浏览器的**强制同步重排（Reflow）**，阻塞主线程。`IntersectionObserver` 将几何交叉计算交由浏览器底层在渲染合成阶段异步完成，大幅提升滚动性能。
2. **核心配置参数**：
   - `root`：监听的容器元素（默认为顶级视口 `null`）。
   - `rootMargin`：类似于 CSS 的 margin，用于扩大或收缩判定边界（如 `"100px 0px"` 实现提前 100px 预加载）。
   - `threshold`：触发回调的交叉比例阈值数组，取值范围 `0.0 ~ 1.0`（如 `[0, 0.5, 1.0]`）。
3. **常见应用场景**：
   - 图片/组件懒加载。
   - 瀑布流无限滚动（触底加载）。
   - 广告/内容真实曝光埋点统计。
...
const observer = new IntersectionObserver((entries, observerInstance) => {
  entries.forEach(entry => {
    // entry.isIntersecting 为 true 表示元素已进入视口
  ...
  root: null, // 相对于视口
  rootMargin: '200px 0px', // 提前 200px 预热加载
  threshold: 0.01 // 只要出现一点点即触发
})
...
**面试官视角**
- 考察性能认知：能否准确指出传统 `scroll + getBoundingClientRect` 导致强制重排的性能隐患；能否熟练说明 `rootMargin` 预热加载策略与 `unobserve` 回收资源的重要性。
