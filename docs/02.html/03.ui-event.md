...
**核心概念**
实现鼠标拖拽弹窗或悬浮元素，本质是监听并协调鼠标事件流：
1. **`mousedown`（按下）**：在目标元素或标题栏按下时，记录当前鼠标指针相对元素左上角的偏移量（`offsetX = e.clientX - rect.left`），开启拖拽标志 `isDragging = true`。
2. **`mousemove`（移动）**：在 `document`（而非仅元素本身）上监听移动事件，当处于拖拽状态时，实时计算新位置：`left = e.clientX - offsetX`，并更新元素绝对/固定定位样式。
3. **`mouseup`（抬起）**：在 `document` 上监听鼠标释放，清理 `isDragging` 标志并解绑移动事件。
...
function makeDraggable (element, handle = element) {
  ...
  const onMouseMove = (e) => {
    ...
    const nextLeft = Math.min(Math.max(0, initialLeft + dx), maxLeft)
    const nextTop = Math.min(Math.max(0, initialTop + dy), maxTop)
...
**面试官视角**
- 考查边界处理：为什么 `mousemove` 和 `mouseup` 必须绑定到 `document` 而不是拖拽元素本身？（因为鼠标移动过快会滑出元素，导致丢失事件卡顿）。
- 加分项：考虑视口边缘限制（Boundary clamping）与文字选中阻止（`e.preventDefault()`）。

</Answer>

## 要实时统计用户浏览器窗口大小，该如何做 {#p1-window-resize}

<Answer>

**核心概念**
实时获取并响应浏览器视口尺寸变化，核心技术方案如下：
1. **`window.onresize` / `resize` 事件**：当窗口宽高发生变化时触发，读取 `window.innerWidth` 和 `window.innerHeight`。
2. **频率控制（节流）**：resize 事件高频连续触发（每秒可达几十次），若在回调中进行复杂 DOM 重排或接口计算，必须通过 `throttle` 降低处理频率。
3. **现代推荐方案（`ResizeObserver`）**：若需精准监听具体 DOM 容器（非全局 window）的盒模型尺寸变化，推荐使用原生的 `ResizeObserver` API。

**手写示例：**

```javascript
function initWindowResizeTracker (callback, wait = 100) {
  let timer = null
...
// 使用
const stopTracking = initWindowResizeTracker((dim) => {
  console.log('窗口尺寸变更:', dim)
})
```

**面试官视角**
- 考查事件性能开销控制与监听器的生命周期解绑（避免 SPA 单页应用中组件卸载后造成内存泄漏）。
