...
|**new.target**|this 指向当前构造函数的实例对象|new.target 只能在构造函数中使用，指向调用该构造函数的实例|
...
1. **调用方式不同**：构造函数通过 `new` 关键字调用，而普通函数直接调用。
2. **返回值不同**：构造函数返回一个新创建的对象，而普通函数返回 `undefined`（如果没有显式返回值）。
3. **上下文不同**：构造函数中的 `this` 指向新创建的对象，而普通函数中的 `this` 指向全局对象（在非严格模式下）或 `undefined`（在严格模式下）。
