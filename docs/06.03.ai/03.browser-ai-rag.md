# 端侧 AI 与智能体检索增强 (Browser AI & RAG)✅

本模块涵盖浏览器端侧（In-Browser / Edge）轻量大模型运行原理、WebGPU 硬件加速与 WebLLM/Transformers.js 架构、前端本地 RAG（检索增强生成）全流程落地（分块、向量化与余弦相似度计算）以及 Agent 长会话上下文滑动窗口压缩与多级记忆管理体系。
