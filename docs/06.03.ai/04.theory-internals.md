# 底层原理与机制 (Underlying Theory & Internals)✅

本模块深挖大模型在前端与端侧落地的底层原理与极限性能机制：浏览器端 WebGPU Compute Shader 硬件加速与模型推理管线、长会话上下文预算与多级记忆管理收敛机制，以及从 GPU 显存注意力机制到前端编排的 Prompt Caching 与 KV-Cache 状态优化。
...
## 什么是 Prompt Caching（前缀缓存）与 KV-Cache 状态优化？静态与动态上下文编排如何降低时延与成本？ {#p0-prompt-caching-kv-cache}
