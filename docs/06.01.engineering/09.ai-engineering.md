...
## 团队引入 Cursor / Copilot 等 AI 工具后，如何构建代码审查质量门禁与防范幻觉？ {#p0-ai-coding-quality-gate}
...
## 前端如何架构大模型 Agent 交互界面？流式渲染（SSE）、MCP 工具调用与决策轨迹如何落地？ {#p0-agent-frontend-mcp-visualization}
...
## 前端如何实现端侧 RAG 本地向量检索？分块策略与上下文窗口如何控制？ {#p0-rag-frontend-vector-search}
...
## 大模型生成式 UI（Generative UI）如何安全渲染？如何防范 XSS 注入与样式污染？ {#p0-generative-ui-sandbox}
...
## Agent 长会话场景下，如何设计多级上下文记忆与滑动窗口压缩机制？ {#p1-agent-memory-context-compression}
