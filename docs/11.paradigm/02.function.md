...
## 函数副作用具体指什么 {#p0-effecitve}

<Answer>

`副作用（side effect）` 是指函数或表达式在执行过程中对外部环境产生的影响。典型的副作用包括对全局变量修改、I/O 操作、状态改变等。

在函数式编程中需要尽量避免产生副作用，通过纯函数的方式，根据输入返回一个确定的输出，不修改外部环境的状态。
...
## 什么是高阶函数，它和柯里化有什么区别？ {#p1-higher-order}

<Answer>

高阶函数(High Order Function 简称 HOF )是指接受一个或多个函数作为参数，或者返回一个函数的函数。它是函数式编程的核心概念之一。柯里化是高阶函数的一种特殊形式，它将多参数函数转换为一系列单参数函数的链式调用。

高阶函数的典型应用包括：

1. 数据处理与转换，例如 `map`、`filter`、`reduce` 本质上是对数组进行操作的高阶函数
2. 行为定制与配置，例如 `setTimeout`、`addEventListener`，包括 `debounce`、`throttle` 等函数可以接受回调函数作为参数，来自定义行为
3. 函数组合与复用，例如 `compose`、`pipe` 等函数可以将多个函数组合成一个新函数，此外 React 中  `HOC`（高阶组件）本质也是高阶函数。

:::tip

从概念上常规函数本质是对值的操作，而高阶函数将函数作为值，这个时候你可以把函数理解为一个操作，而高阶函数实现了对操作的封装和控制，从而实现了复杂的操作编排。比如 `setTimeout` 是对函数延迟执行的操作封装、而 `debounce` 是对函数防抖的操作封装，`filter` 是对数组过滤的操作封装。此外你也可以抽象操作的调度逻辑，比如 `pipe` 实现了操作的串行控制。正是因为高阶函数的存在，函数式编程才有了更强大的表达能力。在实际函数式开发中你可以问自己如下问题来优化组织逻辑:

1. 这个函数是处理的值还是操作？
2. 这个操作是否可以通过其他原子操作结合编排实现？
3. 这个操作是否可以被抽象成一个高阶函数？
