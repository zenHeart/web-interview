...
```json
{
  "compilerOptions": {
    ...
    "module": "ESNext",                    // 模块系统
    "lib": ["DOM", "ES2020"],              // 包含的库文件
    "moduleResolution": "node",            // 模块解析策略
    
    // 输出配置
    "outDir": "./dist",                    // 输出目录
    "outFile": "./bundle.js",              // 单文件输出(仅AMD/System)
    "rootDir": "./src",                    // 根目录
    ...
    "typeRoots": ["./node_modules/@types", "./types"], // 类型根目录
...
function example(name: string, callback?: () => void) {
  ...
  // if (callback()) return; // ❌ 错误：callback可能为undefined
  if (callback && callback()) return; // ✅ 正确
  ...
  if (name) {
    return "Hello " + name; // 必须在所有路径都有返回值
  }
  return "Hello"; // ✅ 必需的返回语句
}

// 2. target的影响
// target: "ES5" - 编译为ES5兼容代码
const arrow = (x: number) => x * 2;
// 编译后: function(x) { return x * 2; }

// target: "ES2015" - 保留箭头函数
// 编译后: const arrow = (x) => x * 2;

// 3. module的影响
export const utils = { add: (a: number, b: number) => a + b };

// module: "CommonJS"
// 编译后: exports.utils = { add: function(a, b) { return a + b; } };

// module: "ESNext" 
// 编译后: export const utils = { add: (a, b) => a + b };
```
