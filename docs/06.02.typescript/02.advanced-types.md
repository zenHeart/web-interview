...
let myFunc: GenericIdentityFn<number> = myIdentity;
...
let result = combine("hello", 42); // [string, number]
...
let stringContainer: Container = { value: "hello" };        // T默认为string
let numberContainer: Container<number> = { value: 42 };     // T指定为number
```
...
type T3 = TypeName<string | number>; // "string" | "number" (分布式)
...
type AsyncNumber = PromiseValue<Promise<number>>; // number
...
type Split1 = SplitString<"hello">;     // ["h", "ello"]
type Split2 = SplitString<"ab">;        // ["a", "b"]
type Split3 = SplitString<"">;          // never
