...
let uncertain: unknown = "type-safe any";
let impossible: never; // 永远不存在的值
function log(): void { console.log("no return"); }
...
// uncertain.length; // 错误！必须先类型检查
if (typeof uncertain === 'string') {
  console.log(uncertain.length); // 正确
}
...
function exhaustiveCheck(x: string | number): string {
  switch (typeof x) {
    case 'string': return 'string';
    case 'number': return 'number'; 
    default: return x; // x是never类型，确保穷尽所有情况
  }
}
...
// 双重断言（谨慎使用）
let num = 42;
// let str = num as string; // 错误！number不能直接断言为string
let str = num as unknown as string; // 强制断言

// 常量断言
const colors = ['red', 'green', 'blue'] as const; // readonly ['red', 'green', 'blue']
let obj = { name: 'Tom', age: 25 } as const; // readonly属性

// 非空断言操作符
function processInput(input?: string) {
  // console.log(input.length); // 错误：可能为undefined
  console.log(input!.length); // 非空断言：确定input不为空
}

// 类型断言 vs 类型转换
let value: any = "123";
let num1 = value as number; // 断言，运行时还是字符串
let num2 = Number(value);   // 转换，运行时变为数字
```
