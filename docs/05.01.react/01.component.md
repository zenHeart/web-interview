...
## React 有哪些内置组件？ {#p0-react-built-in-components}

<Answer>

React 有两个核心包

* [react](https://react.dev/reference/react) 暴露了脱离平台的核心 API 和功能
* [react-dom](https://react.dev/reference/react-dom) 提供了与 DOM 相关的 API 和功能
...
组件|说明|
---|---
`div等组件`| 例如 ref 实现获取组件实例，className 实现类名绑定等，事件会被代理为合成事件|
`<form>` | action 属性除了 url, 还支持函数形式，提交时会调用函数并传入表单数据|
`<input>`| formAction 除了 url 同样支持函数，表单上事件会被代理为合成事件|

</Answer>

## 说一下 Suspense 和 lazy 的使用场景？ {#p0-suspense-lazy}
...
import LazySuspenseApp from '!!raw-loader!./answers/BuiltInComponents/LazySuspense.tsx';
import LazyComponent from '!!raw-loader!./answers/BuiltInComponents/LazyComponent.tsx';
...
<Sandpack
  ...
  template="react-ts"
  files={{
   "/App.tsx": LazySuspenseApp,
   "/LazyComponent.tsx": LazyComponent,
  }}
/>

</TabItem>

<TabItem value="Suspense">

import SuspenseLazyApp from '!!raw-loader!./answers/BuiltInComponents/Suspense.tsx';
...
<Sandpack
  ...
  template="react-ts"
  files={{
   "/App.tsx": SuspenseLazyApp,
  }}
/>

:::tip

一个典型的误区是认为 Suspense 组件针对异步请求加载场景，通过上例可以看出 Suspense 是基于特殊的返回结构来判断加载 callback 还是真实的 children 的。核心使用场景包括

| 使用场景                 | 备注 |
| -------------------- | ----------- |
| `React.lazy()` 加载组件  | -|
| `use()` + 异步函数       | 在服务端组件
| 异步资源手动挂起（抛出 promise） | 模拟异步加载组件 |
...
* [Suspense 官方说明](https://17.reactjs.org/docs/concurrent-mode-suspense.html)
* [Suspense 使用文档](https://react.dev/reference/react/Suspense)
...
## 说一下 ViewTransition 的使用 ? {#p1-view-transition}

<Answer>

**延伸阅读**

* [Add `<ViewTransition>` Component](https://github.com/facebook/react/pull/31975) issue 说明此组件
...
## jsx 返回 null undefined false true 区别？ {#p2-jsx-return-null-undefined-false}
...
## JSX 如何渲染 HTML 注释节点 ？ {#p4-jsx-render-html-comment}

<Answer>

可以采用 [dangerouslySetInnerHTML](https://legacy.reactjs.org/docs/dom-elements.html#dangerouslysetinnerhtml) ，新版文档参见 [Dangerously setting the inner HTML](https://react.dev/reference/react-dom/components/common#dangerously-setting-the-inner-html)
...
* [react 中 mode 是什么？](./05.theory.md#p1-mode)
