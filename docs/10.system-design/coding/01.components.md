...
## Dialog/Modal 如何实现焦点管理与可访问性？ {#p0-dialog-modal}
...
function Modal({ open, onClose, titleId, children }) {
  ...
  if (!open) return null
  return createPortal(
    <div className="mask" aria-hidden onClick={(e) => e.target === e.currentTarget && onClose?.()}>
      <div role="dialog" aria-modal="true" aria-labelledby={titleId} ref={ref}>
        {children}
      </div>
    </div>,
    document.body,
...
## Select/Autocomplete 大数据与搜索体验 {#p1-select}
...
## Cascader 级联选择的懒加载与展开策略 {#p2-cascader}
...
## Pagination 分页的 UX 与 SEO {#p2-pagination}
...
## DatePicker/RangePicker 范围选择与禁用规则 {#p2-datepicker}
...
## Upload 上传（分片/断点续传/并发控制） {#p1-upload}
...
## Message/Notification 全局提示的队列与防抖 {#p2-message}
...
## Popover/Popconfirm 定位与碰撞翻转 {#p1-popover}
...
## Affix/Sticky 吸顶的滚动监听与容器边界 {#p2-affix}
...
## Skeleton/Spin 加载与骨架屏策略 {#p2-skeleton}
