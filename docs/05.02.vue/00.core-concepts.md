...
## Vue2 与 Vue3 的主要区别是什么？{#p0-vue2-vue3-diff}
...
## 什么是编译器宏？ {#p1-compiler-macro}

<Answer>

宏的概念在 C 语言中是一个常用的概念，本质是通过预处理器对定义的文本片段进行替换，Vue 的模版编译借鉴了此思想。
比如 `defineProps()、defineEmits() 和 defineExpose()` 等本质并不是一个函数，而是会被编译器识别后替换为对应 option 配置， 所以不能够采用 `const dp = defineProps` 方法创建别名。基于这个思想有 [Vue Macros](https://vue-macros.dev/) 库来定义一些语法糖。
