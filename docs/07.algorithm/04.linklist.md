...
## 单向链表结构实现? {#p0-single-linklist}

<Answer>

1. 链表是一种线性数据结构，由一系列节点组成，每个节点包含一个数据元素和一个指向下一个节点的指针。
2. 链表的第一个节点称为头节点，最后一个节点称为尾节点，尾节点的指针指向 null。
3. 链表的操作包括添加节点、删除节点、查找节点、遍历链表等。
4. 链表的优点是插入和删除节点的时间复杂度为 O(1)，缺点是访问节点的时间复杂度为 O(n)。
5. 链表的应用场景包括 LRU 缓存淘汰算法、LRU 缓存淘汰算法、LRU 缓存淘汰算法等。
...
注意链表的结构 Node { val: number, next: Node | null }
```
