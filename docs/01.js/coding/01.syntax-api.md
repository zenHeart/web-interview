...
import customJsonTest from '!!raw-loader!./answers/json/json.test.js';
import customJson from '!!raw-loader!./answers/json/json.js';
...
**核心概念**
Promise 是异步编程的一种解决方案，遵循 Promises/A+ 规范。它本质是一个状态机，具有以下核心特性：
1. **三种状态**：`pending`（进行中）、`fulfilled`（已成功）和 `rejected`（已失败），状态一旦改变不可逆转。
2. **决议机制**：通过 `resolve` 和 `reject` 函数改变状态并传递结果或拒绝原因。
3. **链式调用**：`then` 方法返回一个新的 Promise，支持链式异步编排，并通过 Promise 解决程序（Promise Resolution Procedure）递归展开嵌套的 Promise / Thenable。
4. **异步微任务执行**：回调函数必须在当前执行栈清空后的微任务（如 `queueMicrotask` 或 `MutationObserver`/`setTimeout`）中异步执行。
...
- 加分项：能主动说明为什么 then 返回新 Promise 而非 `this`，以及 `queueMicrotask` 相对于 `setTimeout` 的微任务优势。

**延伸阅读**
- [Promises/A+ 官方规范](https://promisesaplus.com/)
- [ECMA-262 Promise Objects 规范](https://tc39.es/ecma262/#sec-promise-objects)
...
1. **全胜则胜**：当所有输入的 Promise 都兑现（fulfilled）时，返回的 Promise 兑现，其值为所有输入结果按原顺序组成的数组。
2. **一败则败（快速失败）**：只要有一个 Promise 拒绝（rejected），返回的 Promise 立即以该拒绝原因拒绝。
3. **空集合处理**：传入空可迭代对象时，同步返回已兑现为空数组的 Promise。
...
**面试官视角**
- 常见考察陷阱：使用 `results.length === len` 判断完成情况（错误，因为数组按索引直接赋值会导致中间空位也占据 length），必须使用独立的计数器 `completedCount`。
- 加分项：支持非数组的可迭代参数（`Array.from(iterable)`），且对每个元素包裹 `Promise.resolve`。
...
- 若传入空数组，返回的 Promise 将永远处于 `pending` 状态（符合规范）。
...
1. `callback` 不接受任何参数。
2. 保持原状态透传：如果原 Promise 成功，finally 会把成功值继续往下传；若原 Promise 失败，继续向下抛出原 reason。
3. 若 `callback()` 返回一个 Promise，finally 会等待该 Promise 决议完毕后再向下透传原值；若 callback 内部抛出异常或返回 rejected Promise，则最终以该新错误拒绝。
...
**面试官视角**
- 容易忽略的细节：`P.resolve(callback()).then(...)` 保证了即使 callback 是异步 Promise 也能正确等待其完成，同时保持原本的 value/reason 干净透传。
...
**核心概念**
`async/await` 是 Generator 函数与 Promise 的语法糖，核心思想是**协程（Coroutine）的自动执行器**（如 co 模块）：
- Generator 函数暂停在 `yield` 处，将控制权交还给自动执行器。
- 自动执行器监听 `yield` 返回的 Promise 状态，在其 resolve 时调用 `iterator.next(value)` 将结果注入回生成器，在其 reject 时调用 `iterator.throw(reason)` 抛出异常。
...
**面试官视角**
- 考查从生成器、协程到现代 async/await 的技术演进理解；要求熟练手写 `step` 递归推进逻辑及异常捕获。

**延伸阅读**
- [Babel 编译 async/await 原理分析](https://babeljs.io/docs/babel-plugin-transform-async-to-generator)
...
1. **this 绑定**：`context` 为 `null` 或 `undefined` 时，`this` 默认指向全局对象（浏览器下为 `window`）。
2. **参数处理**：`apply` 接收参数数组，`call` 依次传递参数。
3. **属性冲突**：临时属性名应唯一，推荐用 `Symbol` 防止覆盖原有属性。
4. **返回值**：需返回原函数的执行结果。
5. **严格模式**：严格模式下 `this` 为 `null` 时不会自动指向全局对象。
...
1. **创建空对象**：创建一个全新的普通 JavaScript 对象，并将其原型（`[[Prototype]]`）链接到构造函数的 `prototype` 属性。
2. **绑定 this 并执行**：将新创建的对象作为 `this` 上下文，执行构造函数代码，初始化属性与方法。
3. **返回值裁决**：检查构造函数的返回值。如果构造函数显式返回了一个**对象类型**（非原始值），则最终返回该对象；否则返回第一步创建的新对象。
...
**面试官视角**
- 考查重点在于原型链继承的串联（`Object.create` 或 `Object.setPrototypeOf`）以及对返回值的严格判断（必须区分 `null` 与真实引用类型）。

**延伸阅读**
- [MDN: new 运算符](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/new)
- [ECMA-262 [[Construct]] 内部方法规范](https://tc39.es/ecma262/#sec-ecmascript-function-objects-construct-argumentslist-newtarget)
