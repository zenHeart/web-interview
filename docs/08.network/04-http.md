...
## 常见的 HTTP 请求头和响应头有哪些？ {#p1-http-headers}
...
- **GET**：搜索、分页、获取资源等读取操作
- **POST**：表单提交、文件上传、创建资源等写入操作

</Answer>

## HTTP 常见状态码有哪些？ {#p0-http-status-codes}
...
![HTTPS参与者关系图](https://user-images.githubusercontent.com/22188674/221914484-98831a29-4af5-472c-be59-f54dcd46b15d.png)
...
![HTTPS工作流程图](https://user-images.githubusercontent.com/22188674/221914949-a44091ea-57f5-49b4-87ad-b777ca1313c3.png)
...
| 头部字段 | 作用 | 示例 |
|----------|------|------|
| **Host** | 指定目标服务器的域名或 IP 地址 | `Host: www.example.com` |
| **User-Agent** | 发送请求的用户代理（浏览器标识） | `User-Agent: Mozilla/5.0...` |
| **Accept** | 指定客户端可以接受的内容类型 | `Accept: text/html,application/json` |
| **Content-Type** | 指定请求体的媒体类型 | `Content-Type: application/json` |
| **Authorization** | 提供身份验证凭据 | `Authorization: Bearer token` |
| **Cookie** | 包含服务器设置的 Cookie | `Cookie: session=abc123` |
| **Referer** | 指定当前请求的来源页面 URL | `Referer: https://example.com/page` |

**常见响应头（Response Header）**

| 头部字段 | 作用 | 示例 |
|----------|------|------|
| **Content-Type** | 指定响应体的媒体类型 | `Content-Type: application/json` |
| **Content-Length** | 指定响应体的长度（字节） | `Content-Length: 1024` |
| **Cache-Control** | 指定缓存策略 | `Cache-Control: max-age=3600` |
| **Set-Cookie** | 在客户端设置 Cookie | `Set-Cookie: session=abc123; HttpOnly` |
| **Location** | 指定重定向的目标 URL | `Location: https://example.com/new` |
| **Access-Control-Allow-Origin** | 指定允许跨域请求的来源 | `Access-Control-Allow-Origin: *` |
| **ETag** | 指定实体标签，用于缓存验证 | `ETag: "686897696a7c876b7e"` |

**Content-Type 详解**

Content-Type 是HTTP头部字段之一，用于指示请求或响应中实体的媒体类型，使用MIME（Multipurpose Internet Mail Extensions）类型标识。

**常见 Content-Type 值**

| 类型 | 用途 | 示例 |
|------|------|------|
| `text/plain` | 纯文本，默认ASCII编码 | 文本文件 |
| `text/html` | HTML文档 | 网页内容 |
| `text/css` | CSS样式表 | 样式文件 |
| `application/json` | JSON数据 | API数据交换 |
| `application/xml` | XML数据 | 结构化数据 |
| `application/octet-stream` | 二进制流数据 | 文件下载 |
| `multipart/form-data` | 表单数据（含文件） | 文件上传 |
| `image/jpeg`, `image/png` | 图像文件 | 图片资源 |

这些头部字段提供额外信息、控制请求和响应行为，实现身份验证、缓存控制、安全性等功能。
...
## 缓存中 Expires 和 Cache-Control 有什么区别？ {#p1-expires-vs-cache-control}
...
在实际应用中建议使用 Cache-Control，因为它更加灵活、可控且不依赖时间同步。如需兼容老旧浏览器，可以同时设置两个头部。
