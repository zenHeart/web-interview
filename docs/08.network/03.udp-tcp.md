...
## TCP 报文的组成和结构字段解析？ {#p0-tcp-packet-structure}
...
## TCP 三次握手流程，为什么需要三次握手？ {#p0-tcp-three-way-handshake}
...
## TCP 四次挥手流程，为什么需要四次挥手与 TIME_WAIT 状态？ {#p0-tcp-four-way-handshake}
...
**核心概念**
TCP 协议作为可靠传输协议，其**重传机制**是解决网络丢包、乱序与损坏的核心保障。主要包括以下四大机制：
1. **超时重传（RTO, Retransmission TimeOut）**：
   - 发送方发送报文段后启动重传定时器。若在定时器超时前未收到对应 ACK，则判定报文丢失并触发重传。
   - **RTO 动态自适应**：RTO 并非固定值，而是基于网络实际的往返时间（RTT）通过 Jacobson/Karels 算法加权平滑计算得出（$RTO = SRTT + 4 \times RTTVAR$）。
   - **指数退避（Karn 算法）**：每次发生超时重传，下一次的超时时间直接翻倍（$RTO \times 2$），以防止在网络严重拥塞时雪上加霜。
2. **快速重传（Fast Retransmit）**：
   - 超时重传等待时间较长。快速重传以**数据驱动**：当接收方收到乱序数据时，按规则重复发送上一个正确连续报文的 ACK。
   - 发送方如果**连续收到 3 个相同的冗余确认（Duplicate ACKs）**，便可断定该报文已丢失，**立即触发重传**，无需等待超时定时器到期。
3. **选择性确认（SACK, Selective Acknowledgment）**：
   - 传统快速重传只知道某个序号丢失，不知道后续报文哪些已经到达。
   - SACK 在 TCP 报头选项字段中填入已收到的不连续数据区间（如 `ACK=1000, SACK=2000-3000`）。发送方可据此**只重传真正丢失的报文段**，大幅节省网络带宽。
4. **重复 SACK（D-SACK, Duplicate SACK）**：
   - 接收方通过 SACK 选项告知发送方收到了重复报文，使发送方能精确判断是“网络延迟乱序”还是“ACK 丢失”，进而撤销不必要的拥塞窗口缩减。

**机制对比总览：**

| 机制 | 触发条件 | 优点 | 缺点/适用场景 |
| :--- | :--- | :--- | :--- |
| **超时重传 (RTO)** | 定时器超时（没收到任何后续 ACK） | 兜底保障，任何丢包均有效 | 延迟大，等待时间长 |
| **快速重传** | 收到 3 个连续 Duplicate ACK | 响应迅速，秒级重传 | 无法精确获知后续多个报文的接收状态 |
| **SACK** | TCP 选项协同告知离散区间 | 精确补发丢失段，吞吐量高 | 依赖双方协商支持，报头空间消耗大 |

**面试官视角**
- 考查深度：为什么是“3 次重复 ACK”才触发快速重传？（因为网络轻微乱序通常在 1-2 个包内恢复，3 次可以大概率排除乱序并确认为真实丢包）。
- 了解 SACK 和拥塞控制（Fast Recovery）的联动。
