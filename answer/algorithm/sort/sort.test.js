    ...
    describe('插入排序', function() {
        for (let unitTestName in testData) {
            it(unitTestName, function() {
                ...
                let res = sort.insertSort(data.input);
                expect(res).to.deep.eq(data.expect);
            });
        }
    });
    describe('选择排序', function() {
        for (let unitTestName in testData) {
            it(unitTestName, function() {
                ...
                let res = sort.selectionSort(data.input);
                expect(res).to.deep.eq(data.expect);
            });
        }
    });
    describe('归并排序', function() {
        describe('归并逻辑测试', function() {
            ...
            for (let unitTestName in testData) {
                it(unitTestName, function() {
                    let data = testData[unitTestName];
                    let res = sort.mergeArr.apply(this, data.input);
        ...
        describe('归并测试', function() {
            for (let unitTestName in testData) {
                it(unitTestName, function() {
                    ...
                    let res = sort.mergeSort(data.input);
                    expect(res).to.deep.eq(data.expect);
                });
            }
        });
    ...
    describe('快速排序', function() {
        for (let unitTestName in testData) {
            it(unitTestName, function() {
                ...
                let res = sort.quickSort(data.input);
                expect(res).to.deep.eq(data.expect);
            });
        }
    });
